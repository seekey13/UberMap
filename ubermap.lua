...
-- itself reads, is gpn.GP in lib/gpnav.lua, next to the dispatch that answers
-- with them.
...
--
-- The dispatch that decides whose a press is -- gpn.press, gpn.pad, gpn.key,
-- gpn.state and the two button tables they read -- lives in lib/gpnav.lua
-- instead, where the input tests can drive the real thing.  What is left here
-- is the half that needs the map: the list, the viewport and the draw.
local nav = { };
...
* asking is an Ashita call, which lib/gpnav.lua is kept clear of so the input
* tests can run outside the game.
--]]
nav.h = {
    show       = show,
...
* separate path from the Tab gpn.press reads off DirectInput.
--]]
...
--[[
* event: xinput_button
* desc : Whose the button is, and so whether the client sees it, is
*        gpn.pad -- which the pad's own self-check drives directly.  All that
*        is left here is turning its answer into the event's blocked flag.
--]]
ashita.events.register('xinput_button', 'ubermap_xinput', function (e)
    if (gpn.pad(ui, e.button, e.state, nav.h)) then
        e.blocked = true;
    end
end);
...
*        order.  Whose a key is, and what it does, is gpn.key and the gpn.press
*        under it; here again all that is left is the blocked flag.
*
...
ashita.events.register('key_data', 'ubermap_key_data', function (e)
    if (not gpn.key(ui, e.key, e.down, nav.h)) then
        return false;
    end
    e.blocked = true;
    return true;
...
*        looks at the buffered stream to learn that.  gpn.state does the wiping;
*        the cast is all that has to happen here, since the buffer is raw bytes
*        and lib/gpnav.lua is kept clear of ffi.
--]]
ashita.events.register('key_state', 'ubermap_key_state', function (e)
    gpn.state(ui, ffi.cast('uint8_t*', e.data_raw), nav.h);
end);
