...
- **XInput button indices** are the bit positions Ashita's `xinput_button` event delivers: `0` D-pad up, `1` down, `2` left, `3` right, `12` A, `13` B. These are the six the map reads; every other button stays the client's. The favorites widget reads four of them — the six minus left and right.
...
- **No toggle.** The map takes the six buttons whenever it is on screen. Unlike the widget, which appears unbidden while you stand somewhere, the map is a thing you deliberately opened and it covers 90% of the screen. If a toggle is wanted later it is a `cfg.gpad` flag and one `and cfg.gpad` in the handler.
